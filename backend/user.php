...
switch ($_POST['type']) {
    ...
    case "logout": // 登出模块
        ...
        break;
    case "confirm_email_sendmail": // 发送邮件验证码模块
        if (!isset($_POST['token']) || empty($_POST['token'])) {
            $data = array('code' => 400, 'message' => 'Missing value(s).');
            $json = json_encode($data);
            exit($json);
            ...
            if (confirm_login($_POST['token'])) {
                ...
                if (search_confirm_key($_POST['token'])) {
                    ...
                    if ($old_access_key != $access_key) { // 判断是否发出过（节省api次数
    ...
    case "change_passwd": // 修改密码模块
        ...
        break;
    case "forgot_passwd_sendmail": // 发送账号找回邮件模块
